# dynamic-bot-agent

基于 Dynamic-bot 开发的插件，集成了一些自用功能。欢迎使用[dynamic-bot](https://github.com/Colter23/dynamic-bot)！

基于 DeepSeek API 的多功能 Agent 插件，为 dynamic-bot 提供 AI 对话、Dota2 战报分析、联网搜索能力。

## 功能

- **AI 对话** — 多轮对话，支持记忆（轮数/Token/TTL 三层保护），长文本自动渲染为图片
- **联网搜索** — ReAct Agent 模式，三层搜索后备 (Exa MCP → Parallel MCP → DuckDuckGo)
- **Dota2 战报** — OpenDota API + DeepSeek AI 分析，MVP/战犯识别，表格图片渲染
- **Dota2 历史** — 近期战绩卡片，英雄头像、彩色 K/D/A、胜负汇总与实时序号查询
- **Dota2 分析** — 双阵营全分析
- **Dota2 个人详情** — 最近十场统一统计、多维分析、基准雷达、逐场趋势、英雄分布与两场代表局；普通/深度两种模式

## 效果演示

以下为玩家 `176496411` 的实际输出示例，点击图片可放大。截图中的战绩是生成当时的数据，实时查询会随新对局变化。

### 最近10场历史

`/dota 历史 176496411`：查看该玩家最近10场战绩，最新一场排在最上方。

[![指定玩家最近10场历史](docs/images/dota-history.webp)](docs/images/dota-history.webp)

### 单场战报

`/dota 战报 176496411 1`：分析该玩家最新一场比赛；序号可填写 `1～10`。下图对应截图当时的第1场。

[![指定玩家单场战报与锐评](docs/images/dota-report.webp)](docs/images/dota-report.webp)

### 最近10场个人详情

`/dota 个人详情 176496411`：汇总最近10场表现，展示雷达、趋势、多维锐评和代表对局。

<details>
<summary>展开个人详情完整长图</summary>

[![指定玩家最近10场个人详情分析](docs/images/dota-overview.webp)](docs/images/dota-overview.webp)

</details>

## 命令

| 命令 | 说明 |
|------|------|
| `/ds <问题>` | 发起 AI 对话 |
| `/ds clear` | 清空上下文 |
| `/dota 绑定 <玩家ID>` | 绑定账号 |
| `/dota 历史 [玩家ID]` | 最近10场战绩 |
| `/dota 战报 [序号]` | 绑定账号的单场战报；不填查最新 |
| `/dota 战报 <玩家ID> [序号]` | 指定玩家的单场战报；不填序号查最新 |
| `/dota 个人详情 [玩家ID]` | 最近10场个人分析 |
| `/dota 战报 比赛 <比赛ID>` | 精确查询绑定账号参与的比赛 |
| `/dota 分析 <比赛ID>` | 双阵营分析 |

**参数说明：** `<参数>` 必填，`[参数]` 可选，输入时不带括号。所有查询（包括指定玩家查询）均需先绑定账号。绑定和帮助无需绑定；不填玩家 ID 时使用绑定账号，指定玩家查询不修改绑定。

**战报序号：** 1～10，1 为最新，与历史图一致，随新对局顺延。战报单参数大于10按玩家 ID 查询最新一场；精确比赛查询改用 `/dota 战报 比赛 <比赛ID>`。记录不足时明确提示。

**深度模式：** 将 `战报`、`个人详情` 替换成 `深度战报`、`深度个人详情` 即可开启思考，参数相同、耗时更长；普通模式关闭思考。

示例：`/dota 历史 176496411`、`/dota 战报 176496411 1`、`/dota 深度个人详情 176496411`。

除命令外，`@Bot 文本` 也可触发对话（需在配置中开启 `trigger.enableAtTrigger`）。

普通/深度战报与普通/深度个人详情在同一插件实例中共用一个任务名额（跨用户、群聊和聊天入口）。已有任务时立即提示等待完成后重新发送，本次请求不排队。名额覆盖查询、分析、绘图和向宿主提交消息，失败或取消后释放。宿主接受投递后释放名额，聊天端的实际送达由宿主负责。

个人详情只抓取一份最近十场快照，最多并发两个详情请求；成功的详情最多缓存 32 场、有效期五分钟，失败不缓存。场均值仅用有效数据，缺失显示 `—`，零死亡仍显示零。代表局按经济、每分钟英雄伤害、建筑伤害三项同英雄百分位均值选取最高与最低；基准不足时不强行评分。雷达和趋势不是整体实力评分，也不能推断位置、技能操作或录像事件。

个人详情主体提示词目标为每段 300–420 字，总览 120–200 字、综合建议 220–320 字；与战报直接复用同一份锐评风格指令，采用递进吐槽与军事/财务/医疗比喻，以数据支撑火力，每段用游戏梗收尾；较长内容按完整句子收束并保留收尾，绘图按实测高度展开。普通和深度模式共用提示词、数据、模型配置与 6144 token 上限，只切换思考开关。先进行一次结构化分析；结构、比赛配对、篇幅或部分事实规则失败时最多修正一次，按章节保留两次响应中已通过检查的内容；修正请求失败不会丢掉首轮可用段落，仍无效的章节标为数据摘要。被接口标记为截断的输出不参与合并。接口完全失败则保留统计报告。规则检查不等于完整语义验证，不能保证模型的所有判断正确。两种模式的耗时均取决于接口响应，不承诺固定秒数。

战报生成时，AI 分析与图片资源准备并行执行；英雄及装备资源加载最多并发 6 个，同份报告的重复资源只解码一次。编码图片字节缓存上限为 16 MiB / 256 条，仍保留磁盘缓存。两种战报只切换本次请求的思考模式，模型、提示词、token 上限和重试策略相同；不会自动切换模式，也不会影响其他用户同时发起的请求。

服务器日志可搜索 `dota_report`：`detail` 为比赛详情，`constants` 为常量准备，`ai` 为分析请求（包含可能的重试），`assets` 为资源准备，`assemble` 为报告组装，`draw` 为绘图，`encode` / `encode_and_send` 为编码或编码发送。`analysis_and_resources` 为常量准备到组装完成的耗时；`ai` 与 `assets` 并行，不应将各阶段直接相加。单队战报日志附带 `mode=normal/deep` 和 `thinking=disabled/enabled`，日志不输出提示词、密钥或模型回复。

## 安装

本插件面向插件 API **3.x** 的 dynamic-bot 宿主，编译依赖 `dynamic-bot-core:0.0.4`，`plugin.yml` 声明 `apiVersion: 3.0.0`。core 依赖版本与插件 API 版本是两套版本号；API 2.x / 4.x 宿主需要对应版本的插件。

```powershell
.\gradlew.bat fatJar
```

把 `build/libs/dynamic-bot-agent-0.0.5-all.jar` 放入宿主 `plugins/` 目录，启动或重启 dynamic-bot 即可。

> 构建前需将 `top.colter.skiko:skiko-layout:0.0.9` 安装到本地 Maven 仓库。

## 配置

插件部署后通过 dynamic-bot 的 Web Admin 进行配置，所有字段均可在 UI 中修改并即时生效：

| 区域 | 配置项 | 类型 | 说明 |
|------|--------|------|------|
| **DeepSeek API** | API Key | SECRET | 必填，从 platform.deepseek.com 获取 |
| | API URL | TEXT | 默认 `https://api.deepseek.com/chat/completions` |
| | 模型 | SELECT | V4 Flash / V4 Pro |
| **对话** | 系统提示词 | TEXTAREA | |
| | 最大 Token | NUMBER | 256-16384 |
| | 图片阈值 | NUMBER | 超过此字数自动渲染为图片，0=始终文本 |
| | 启用记忆 | BOOLEAN | 多轮对话记忆开关 |
| | 记忆轮数 | NUMBER | 1-50 |
| | 会话超时 | NUMBER | 分钟，30 分钟无活动自动清除 |
| | 上下文 Token 限制 | NUMBER | 超限自动踢出早期轮次 |
| **触发** | 触发前缀 | TEXT | 默认 `/ds` |
| | @Bot 触发 | BOOLEAN | 开启后 @Bot 即可对话 |
| **搜索** | 启用联网搜索 | BOOLEAN | ReAct Agent 模式 |
| | 最大轮数 | NUMBER | 工具调用最大轮次 |
| | Tavily API Key | SECRET | 可选，用于 URL 预抓取 |
| **图片** | 主题色 | TEXT | `#667eea;#764ba2` 格式，分号分隔 |
| | 倍率 | NUMBER | 1.0-4.0 |

## 缓存

运行时数据存储在 `data/plugins/dynamic-agent/` 下：

- `dota2-bindings.json` — Dota2 账号绑定
- `cache/draw/` — 渲染图片
- `cache/dota2/icons/` — 英雄/物品/A杖图标
- `fonts/` — 自定义字体 (可选)
